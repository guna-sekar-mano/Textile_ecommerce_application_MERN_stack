export default function Tableheadpanel ({ openform, setGlobalFilter, globalFilter,clearFilter }) {

    return (
        <>
        <div className="flex flex-col md:items-center md:justify-between md:flex-row mb-3">
            <div><h2 className="mb-2 text-2xl font-bold md:mb-0">Products</h2></div>
            <div className="flex items-center gap-4">
                <div className="relative min-w-64 md:min-w-80 flex items-center">
                    ...
                    <input onChange={(e) => setGlobalFilter(e.target.value)} value={globalFilter} type="text" id="icon" name="icon" className="block w-full px-4 py-3 text-sm border border-gray-200 outline-none ps-11 disabled:opacity-50 disabled:pointer-events-none" placeholder="Search Product ..." />
                ...
                <div>
                    <button onClick={openform} className="flex items-center gap-1 px-4 py-2.5 text-sm  shadow-sm bg-black text-white cursor-pointer"><i className="mt-1 fi fi-rr-square-plus text-md"></i> <span className="hidden md:block">Add New Product</span></button>
                </div>
            </div>
        </div>
        </>
