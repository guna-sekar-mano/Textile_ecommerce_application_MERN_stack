...
export default function BannerProducts() {
    ...
    const { bannerId } = useParams(); // This gets the banner name from URL path
    const location = useLocation();
    const navigate = useNavigate();
    const [grid, setGrid] = useState(4);
    const [isOpen, setIsOpen] = useState(false);

    // Extract banner ID from query parameters
    const searchParams = new URLSearchParams(location.search);
    const actualBannerId = searchParams.get('id');
    ...
    const fetchBannerProducts = useCallback(async () => {
        // Use actualBannerId from query params instead of bannerId from URL path
        if (!actualBannerId) return;
        
        setLoading(true);
        try {
            const bannerResponse = await getallCustomerBanner();
            const currentBanner = bannerResponse?.resdata?.find(banner => banner._id === actualBannerId);
            ...
            const allProductsResponse = await apigetallproductsCustomers();
            
            if (allProductsResponse.resdata) {
                let bannerProducts = [];
                
                if (currentBanner.ProductId && currentBanner.ProductId.length > 0) {
                    const productIds = currentBanner.ProductId.map(p => p._id);
                    
                    bannerProducts = allProductsResponse.resdata.filter(product => 
                ...
                const uniqueSizes = extractUniqueSizes(bannerProducts);
                const dynamicPriceRanges = generatePriceRanges(bannerProducts);
                
                setAvailableSizes(uniqueSizes);
                setPriceRanges(dynamicPriceRanges);
                setFilteredProducts(bannerProducts);
            }
        } catch (error) {
    ...
    }, [actualBannerId]); // Changed dependency from bannerId to actualBannerId
                ...
                <div className="max-w-[95rem] mx-auto">
                    <div className='flex flex-col md:flex-row md:justify-between md:items-center'>
                        <div className="flex items-center gap-4 mb-6">
                            {bannerData && (
                                <h1 className="text-2xl azeret-mono">{bannerData.Banner_Name}</h1>
                            )}
                        </div>
                    ...
                    {filteredProducts.length === 0 && !loading ? (
                        <div className="text-center py-10">
                            <p className="text-lg text-gray-600">
                                {selectedSizes.length > 0 || selectedPriceRanges.length > 0 ? "No products found matching the selected filters." : "No products found for this banner."}
                            </p>
